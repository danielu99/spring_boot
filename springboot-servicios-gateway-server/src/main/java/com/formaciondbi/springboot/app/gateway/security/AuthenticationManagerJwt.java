...
public class AuthenticationManagerJwt implements ReactiveAuthenticationManager{
	...
	public Mono<Authentication> authenticate(Authentication authentication) {
		return Mono.just(authentication.getCredentials().toString()).map(token->{
			...
			Collection<GrantedAuthority> authorities = roles.stream().map(rol->new SimpleGrantedAuthority(rol)).collect(Collectors.toList());
			return new UsernamePasswordAuthenticationToken(username,null,authorities);
		}); //JUST convierte un objeto normal y corriente a un objeto reactivo (Mono).
	}
